...
inline void cuda_check_error() {
  ...
  if (status != cudaSuccess) {
    std::cerr << cudaGetErrorName(status) << ": " << cudaGetErrorString(status);
    exit(1);
  }
}
...
inline double rand_real() { return rand_real(0.0, 1.0); }

template <typename T> constexpr T _sqr(T a) { return a * a; }
