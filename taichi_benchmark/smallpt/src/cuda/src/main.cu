...
enum materialType { DIFFUSE = 0, MIRROR, GLASS };
...
struct sphere {
  ...
  __device__ double intersect(const Ray &r) const {
    ...
    float t, epsilon = 0.0001f; // epsilon required to prevent floating point
                                // precision artefacts
    float b = dot(op, r.direction); // b in quadratic equation
    float disc = b * b - dot(op, op) +
                 radius * radius; // discriminant quadratic equation
    if (disc < 0)
      return 0; // if disc < 0, no real solution (we're not interested in
                // complex roots)
    else
      disc = sqrtf(disc); // if disc >= 0, check for solutions using negative
                          // and positive discriminant
    return (t = b - disc) > epsilon
               ? t
               : ((t = b + disc) > epsilon
                      ? t
                      : 0); // pick closest point in front of ray origin
  }
};
...
__device__ bool intersectScene(const Ray &r, float &t, int &id) {
  float n = sizeof(spheres) / sizeof(sphere), d,
        inf = t = 1e20; // t is distance to closest intersection, initialise t
                        // to a huge number outside scene
  for (int i = int(n); i--;) {
  ...
  return t < inf; // returns true if an intersection with the scene occurred,
                  // false when no hit
}

__device__ float clamp(float x) { return x < 0 ? 0 : x > 1 ? 1 : x; }

__device__ float gammaCorrection(float x) { return pow(clamp(x), 1 / 2.2f); }

__device__ float3 radiance(Ray &r, curandState *rs) {
  float3 L = make_float3(
      0.0f, 0.0f,
      0.0f); // accumulates ray colour with each iteration through bounce loop
  ...
  for (int i = 0; i < 10 /*max_depth*/; i++) {
    ...
    float3 nl = dot(normal, r.direction) < 0
                    ? normal
                    : normal * -1; // front facing normal
    ...
    if (obj.type == DIFFUSE) {
      ...
      float3 w = nl;
      float3 u = normalize(cross(
          (fabs(w.x) > .1 ? make_float3(0, 1, 0) : make_float3(1, 0, 0)), w));
      float3 v = cross(w, u);

      // local to world convert
      r.direction =
          normalize(u * cos(r1) * r2s + v * sin(r1) * r2s + w * sqrtf(1 - r2));
      // pdf = 1.0f / PI;
      ...
      if ((cos2t = 1 - nnt * nnt * (1 - ddn * ddn)) < 0) {
        ...
        // refract or reflect
        float3 tdir =
            normalize(r.direction * nnt -
                      normal * ((into ? 1 : -1) * (ddn * nnt + sqrt(cos2t))));

        float a = nt - nc, b = nt + nc, R0 = a * a / (b * b),
              c = 1 - (into ? -ddn : dot(tdir, normal));
    ...
    // Russian roulette Stop with at least some probability to avoid getting
    // stuck
    if (depth++ >= 5) {
...
__global__ void render(int spp, int width, int height, float3 *output) {
  ...
  Ray cam(make_float3(50, 52, 295.6),
          normalize(make_float3(0, -0.042612, -1))); // cam pos, dir
  ...
  // output to the cache
  output[i] =
      make_float3(clamp(color.x, 0.0f, 1.0f), clamp(color.y, 0.0f, 1.0f),
...
int main(int argc, char *argv[]) {
  ...
  cudaMemcpy(outputCPU, outputGPU, width * height * sizeof(float3),
