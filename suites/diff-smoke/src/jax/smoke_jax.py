...
def run_smoke(num_time_steps=25):
    ...
    def d_simulate(vx,
        ...
        for t in range(num_time_steps - 1, -1, -1):
            ...
            d_vx_updated, d_vy_updated = d_project(vx_updated, vy_updated,
                                                   d_vx, d_vy)
    ...
    def plot_matrix(ax, mat, t, render=False):
        ...
        if render:
            matplotlib.image.imsave(
                'output_autograd/step{0:03d}.png'.format(t), mat)
        plt.pause(0.001)
    ...
    def run():
        ...
        for i in range(10):
            ...
            time_ret = np.median(np.array(times))
        return {'steps': num_time_steps, 'time_ms': float(time_ret)}
